...
int main(int argc, char *argv[]){
   ...
   int socket_connected = connect(descriptor, (struct sockaddr *)&server, sizeof(server));
   if(socket_connected != 0){
      printf("%s: error trying to connect\n", argv[0]);
      perror(NULL);
      return 2;
   }
...
void calculate(int *descriptor, struct sockaddr_in *client, struct sockaddr_in *server){
   ...
   while(1){
      ...
      int message_sent = send(*descriptor, 
                               calc_input->message, 
      ...
      if(message_sent == -1){
         ...
         int server_response = recv(*descriptor,
                                    calc_input->message,
                                    sizeof(char)*BUFFER_LENGTH,
                                    0);
